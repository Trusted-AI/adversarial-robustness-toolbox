...
class AsrDecoderBase(base_decoder.BaseBeamSearchDecoder):
    ...
    def Params(cls):
        ...
        p.Define(
            ...
            "RNNCell params template. " "Can be a single param or " "a list of rnn_layers params, one for each layer.",
        )
        p.Define("rnn_cell_dim", 0, "size of the rnn cells.")
        p.Define(
            "rnn_cell_hidden_dim",
            0,
            "internal size of the rnn cells. When "
        ...
        p.Define(
            "token_normalized_per_seq_loss",
            False,
            "Whether or not to normalize the per-sequence loss by the sequence " "length.",
        )
        # Configs for scheduled sampling.
        p.Define(
            "min_ground_truth_prob", 1.0, "The min probability of using the ground truth as the previous " "prediction."
        )
        p.Define("min_prob_step", 1e6, "Step to reach min_ground_truth_prob.")
        p.Define(
            "prob_decay_start_step",
            1e4,
            "The step to starts linearly decrease the probability of sampling " "ground truth.",
        )
        p.Define(
            "use_while_loop_based_unrolling",
            True,
        ...
        p.Define(
            "use_unnormalized_logits_as_log_probs",
            True,
        ...
        p.Define(
            "contextualizer",
            contextualizer_base.NullContextualizer.Params(),
    ...
    def __init__(self, params):
        ...
        if self._decay_interval <= 0:
            raise ValueError(
                "min_prob_step (%d) <= prob_decay_start_step (%d)" % (p.min_prob_step, p.prob_decay_start_step)
            )
        ...
        with tf.variable_scope(name):
            ...
            if p.label_smoothing is not None:
                ...
                if p.label_smoothing.num_classes == 0:
                    ...
                    raise ValueError(
                        "label_smoothing.num_classes ({}) does not match "
                        "softmax.num_classes ({})".format(p.label_smoothing.num_classes, p.softmax.num_classes)
                    )
    ...
    def _InitAttention(self, theta, encoder_outputs):
        ...
        packed_src = self.atten.InitForSourcePacked(
            theta.atten, encoder_outputs.encoded, encoder_outputs.encoded, encoder_outputs.padding  # src  # context
        )
    ...
    def BaseZeroState(self, theta, encoder_outputs, bs, misc_zero_states, per_step_source_padding=None):
        ...
        (atten_context, atten_probs, atten_states) = self.atten.ComputeContextVectorWithSource(
            theta.atten,
            packed_src,
            py_utils.Zeros([bs, self.rnn_cell[0].params.num_output_nodes], dtype=py_utils.FPropDtype(p)),
            zero_atten_state,
    ...
    def DecoderStepZeroState(self, theta, encoder_outputs, target_ids, bs):
        ...
        rnn_states, atten_context, atten_probs, atten_states, packed_src = self.BaseZeroState(
            theta, encoder_outputs, bs, misc_zero_states
        )
        return (
            py_utils.NestedMap(
                ...
                misc_states=misc_zero_states,
            ),
            packed_src,
        )

    def _AddDecoderActivationsSummary(
        self,
        encoder_outputs,
        targets,
        atten_probs,
        ...
        fig = plot.MatplotlibFigureSummary(
            ...
            gridspec_kwargs=dict(width_ratios=[3] + [1] * (num_cols - 1), height_ratios=(4, 1)),
        )
            ...
            if alignments is not None:
                ...
                atten_probs = 1 - tf.stack(
                    [
                        ...
                        tf.minimum(atten_probs + alignment_probs, 1.0),
                        alignment_probs,
                    ],
                    axis=-1,
                )
    ...
    def _ComputeMetrics(self, logits, target_labels, target_weights, target_probs=None):
        ...
        correct_preds = tf.cast(
            tf.equal(tf.argmax(logits, 2, output_type=tf.int32), target_labels), py_utils.FPropDtype(p)
        )
    ...
    def InitDecoder(self, theta, encoder_outputs, dec_bs):
        decoder_step_zero_state, packed_src = self.DecoderStepZeroState(
            theta, encoder_outputs, tf.ones([dec_bs, 1], dtype=tf.int32) * self.params.target_sos_id, dec_bs
        )

        return (
            decoder_step_zero_state.rnn_states,
    ...
    def ComputeLoss(self, theta, predictions, targets):
        ...
        with tf.name_scope(p.name):
            ...
            def AddToMetric(acc, scale, metric):
                ...
                assert len(metric) == 2
                return (
                    acc[0] + scale * tf.cast(metric[0], py_utils.FPropDtype(p)),
            ...
            for logit_name, loss_weight in six.iteritems(p.logit_types):
                metrics, per_sequence_loss = self._ComputeMetrics(
                    getattr(predictions, logit_name), targets.labels, targets.weights, target_probs
                )
                for k, v in six.iteritems(metrics):
                    ...
                    if k not in merged_metrics:
                        merged_metrics[k] = (
                            tf.zeros(shape=[], dtype=py_utils.FPropDtype(p)),
                            tf.zeros(shape=[], dtype=py_utils.FPropDtype(p)),
                        )
    ...
    def _GetInitialSeqStateTensorArrays(
        self, max_seq_length, decoder_step_state_zero_fusion_flat, decoder_step_state_zero_misc_flat
    ):
        ...
        return AsrDecoder.SequenceOutTensorArrays(
            rnn_outs=[
                _NewTensorArray(name="rnn%d_outs" % i, max_seq_length=max_seq_length, dtype=py_utils.FPropDtype(p))
                for i in range(p.rnn_layers)
            ],
            step_outs=_NewTensorArray(name="step_outs", max_seq_length=max_seq_length, dtype=py_utils.FPropDtype(p)),
            atten_probs=_NewTensorArray(
                name="atten_probs", max_seq_length=max_seq_length, dtype=py_utils.FPropDtype(p)
            ),
            logits=_NewTensorArray(name="logits", max_seq_length=max_seq_length, dtype=py_utils.FPropDtype(p)),
            fusion=[
                _NewTensorArray(
    ...
    def _GetPredictionFromSequenceOutTensorArrays(self, seq_out_tas):
        return py_utils.NestedMap(
            ...
            attention=py_utils.NestedMap(probs=self._GetAttenProbsFromSequenceOutTensorArrays(seq_out_tas.atten_probs)),
        )

    def _GetInitialTargetInfo(self, targets, max_seq_length, target_embs):
        return AsrDecoderBase.TargetInfo(
            id=_ToTensorArray("target_ids_ta", tf.transpose(targets.ids), max_seq_length, clear_after_read=False),
            label=_ToTensorArray(
                "target_labels_ta", tf.transpose(targets.labels), max_seq_length, clear_after_read=False
            ),
            weight=_ToTensorArray("target_weights_ta", tf.transpose(targets.weights), max_seq_length),
            emb=_ToTensorArray("target_embs_ta", tf.transpose(target_embs, [1, 0, 2]), max_seq_length),
            padding=_ToTensorArray(
                "target_paddings_ta", tf.expand_dims(tf.transpose(targets.paddings), -1), max_seq_length
            ),
    ...
    def ComputePredictionsDynamic(self, theta, encoder_outputs, targets):
        ...
        with tf.name_scope(p.name):
            ...
            seq_out_tas = self._GetInitialSeqStateTensorArrays(
                max_seq_length, decoder_step_state_zero_fusion_flat, decoder_step_state_zero_misc_flat
            )
            ...
            def _LoopBody(time, old_decoder_step_state, target_info_tas, seq_out_tas):
                ...
                step_outs, decoder_step_state.fusion_states = self.fusion.FProp(
                    theta.fusion,
                    old_decoder_step_state.fusion_states,
                    step_outs,
                    cur_target_info.id,
                ...
                xent_loss = self.softmax.FProp(
                    theta.softmax, [step_outs], class_weights=cur_target_info.weight, class_ids=cur_target_info.label
                )
            ...
            self._AddDecoderActivationsSummary(
                encoder_outputs, targets, seq_out_tas.atten_probs, seq_out_tas.rnn_outs, softmax_input
            )
    ...
    def ComputePredictionsFunctional(self, theta, encoder_outputs, targets):
        ...
        with tf.name_scope(p.name):
            ...
            def RnnStep(recurrent_theta, state0, inputs):
                ...
                with tf.name_scope("single_decode_step"):
                    step_outs, state1 = self.SingleDecodeStep(
                        recurrent_theta.theta, recurrent_theta.packed_src, inputs, state0, use_deterministic_random=True
                    )
            ...
            softmax_input, state0.fusion_states = self.fusion.FProp(
                theta.fusion, state0.fusion_states, step_out, inputs.id, inputs.padding, inputs.misc
            )
            ...
            if isinstance(atten_states, py_utils.NestedMap):
                additional_atten_probs = sorted(
                    [(name, tensor) for name, tensor in atten_states.FlattenItems() if name.endswith("probs")]
                )
            else:
    ...
    def _ComputeLogits(self, theta, softmax_input):
        if isinstance(self.softmax, layers.ConvSoftmax):
            ...
            xent_loss = self.softmax.FProp(
                theta.softmax,
                [softmax_input],
                class_weights=tf.ones(shape=tf.shape(softmax_input)[:-1], dtype=softmax_input.dtype),
                class_ids=tf.ones(shape=tf.shape(softmax_input)[:-1], dtype=tf.int32),
            )
            return xent_loss.logits

    def SingleDecodeStep(
        self,
        theta,
        packed_src,
        cur_target_info,
    ...
    def TargetsToBeFedAtCurrentDecodeStep(self, time, theta, decoder_step_state, target_info_tas, seq_out_tas):
        ...
        if self._max_label_prob > 0:
            ...
            pick_groundtruth = tf.less(
                tf.random_uniform([dec_bs], seed=self.params.random_seed), decoder_step_state.misc_states.groundtruth_p
            )
            emb = tf.where(
                pick_groundtruth,
                target_info_tas.emb.read(time),
                self.emb.EmbLookup(theta.emb, tf.stop_gradient(decoder_step_state.misc_states.prev_predicted_ids)),
            )
            target_id = tf.where(
                pick_groundtruth, target_info_tas.id.read(time), decoder_step_state.misc_states.prev_predicted_ids
            )
...
class AsrDecoder(AsrDecoderBase):
    ...
    def AddAdditionalDecoderSummaries(self, encoder_outputs, targets, seq_out_tas, softmax_input):
        ...
        if cluster_factory.Current().add_summary:
            self.fusion.AddAdditionalDecoderSummaries(
                encoder_outputs.encoded, encoder_outputs.padding, targets, seq_out_tas, softmax_input
            )

    def _ComputeAttention(
        self, theta, rnn_out, packed_src, attention_state, per_step_src_padding=None, query_segment_id=None
    ):
    ...
    def SingleDecodeStep(
        ...
        new_rnn_states_0, _ = self.rnn_cell[0].FProp(
            theta.rnn_cell[0],
            decoder_step_state.rnn_states[0],
            py_utils.NestedMap(
                act=[cur_target_info.emb, decoder_step_state.atten_context], padding=cur_target_info.padding
            ),
        )
        ...
        (new_atten_context, new_atten_probs, new_atten_states) = self._ComputeAttention(
            theta, rnn_out, packed_src, decoder_step_state.atten_states, per_step_src_padding=per_step_src_padding
        )
        ...
        new_atten_context = self.contextualizer.QueryAttention(
            theta.contextualizer, rnn_out, misc_states, new_atten_context, packed_src
        )
        for i, cell in enumerate(self.rnn_cell[1:], 1):
            new_rnn_states_i, _ = cell.FProp(
                theta.rnn_cell[i],
                decoder_step_state.rnn_states[i],
                py_utils.NestedMap(act=[rnn_out, new_atten_context], padding=cur_target_info.padding),
            )
            new_rnn_states.append(new_rnn_states_i)
            new_rnn_out = cell.GetOutput(new_rnn_states_i)
            new_rnn_out = self._ApplyDropout(
                theta, new_rnn_out, deterministic=use_deterministic_random, extra_seed=i * 1000
            )
    ...
    def _InitBeamSearchStateCallback(self, theta, encoder_outputs, num_hyps_per_beam):
        ...
        (
            ...
            misc_states,
            packed_src,
        ) = self.InitDecoder(theta, encoder_outputs, num_hyps)
        ...
        initial_results = py_utils.NestedMap(
            {
                "log_probs": tf.nn.log_softmax(
                    tf.zeros([num_hyps, p.softmax.num_classes], dtype=py_utils.FPropDtype(p))
                ),
                "atten_probs": atten_probs,
            }
        )
    ...
    def _PreBeamSearchStepCallback(self, theta, encoder_outputs, step_ids, states, num_hyps_per_beam):
        ...
        step_out, new_decoder_step_state = self.SingleDecodeStep(
            theta, packed_src, cur_target_info=cur_target_info, decoder_step_state=prev_decoder_step_state
        )
        (atten_context, atten_probs, rnn_states, atten_states, misc_states) = (
            new_decoder_step_state.atten_context,
        ...
        softmax_input, fusion_states = self.fusion.FProp(
            theta.fusion, prev_fusion_states, softmax_input, cur_target_info.id, cur_target_info.padding
        )
