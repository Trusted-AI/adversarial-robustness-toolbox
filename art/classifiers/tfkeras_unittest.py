...
from art.classifiers import TFKerasClassifier
...
class TestTFKerasClassifier(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        k.clear_session()
    ...
    #     classifier = TFKerasClassifier((0, 1), self.model_mnist, use_logits=True)

    # def test_probabilities(self):
    #     classifier = TFKerasClassifier((0, 1), self.model_mnist, use_logits=False)
    ...
    def test_shapes(self):
        ...
        classifier = TFKerasClassifier((0, 1), self.model_mnist)
    ...
    def test_functional_model(self):
        ...
        tfkeras_model = TFKerasClassifier((0, 1), m, input_layer=1, output_layer=1)
        self.assertTrue(tfkeras_model._input.name, "input1")
        self.assertTrue(tfkeras_model._output.name, "output1")
        tfkeras_model = TFKerasClassifier((0, 1), m, input_layer=0, output_layer=0)
        self.assertTrue(tfkeras_model._input.name, "input0")
        self.assertTrue(tfkeras_model._output.name, "output0")
    ...
    def test_layers(self):
        ...
        classifier = TFKerasClassifier((0, 1), model=self.model_mnist)
    ...
    def test_resnet(self):
        ...
        from tf.keras.applications.resnet50 import ResNet50, decode_predictions
        from tf.keras.preprocessing.image import load_img, img_to_array

        tf.keras.backend.set_learning_phase(0)
        model = ResNet50(weights='imagenet')
        classifier = TFKerasClassifier((0, 255), model)
