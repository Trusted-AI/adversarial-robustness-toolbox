...
class TestUtils(unittest.TestCase):
    ...
    def test_to_categorical(self):
        ...
        Y = to_categorical(y)
        self.assertEqual(Y.shape, (6, 10))
        self.assertTrue(np.all(Y.argmax(axis=1) == y))
        self.assertTrue(np.all(np.logical_or(Y == 0.0, Y == 1.0)))
        
        Y = to_categorical(y, 20)
        self.assertEqual(Y.shape, (6, 20))
        
    def test_random_targets(self):
        y = np.array([3, 1, 4, 1, 5, 9])
        Y = to_categorical(y)
        
        rY = random_targets(y,10)
        self.assertTrue(np.all(y != rY.argmax(axis=1)))
        
        rY = random_targets(Y, 10)
        self.assertTrue(np.all(Y.argmax(axis=1) != rY.argmax(axis=1)))
    
    def test_get_label_conf(self):
        y = np.array([3,1,4,1,5,9])
        Y = to_categorical(y)
        
        logits = np.random.normal(10*Y, scale=0.1)
    ...
    def test_preprocess(self):
        ...
        self.assertEqual(Y.shape, (y.shape[0], 10))
        ...
        y = np.argmax(y, axis=1)[:100]
        X, Y = preprocess(x, y, nb_classes=20, max_value=5)
        self.assertEqual(X.shape, x.shape)
