    ...
    def __call__(self, classifier, max_perturb=1.0, scale=0.1, input_layer=0, output_layer=0, streamlined=False):
        ...
        if streamlined:
            if isinstance(classifier, KerasClassifier):
                ...
                raise NotImplementedError("End-to-end network integration of the defense is implemented only for Keras as of now.")
        else:  # Note that the Numpy version makes in-place change to the given classifier.
            classifier._predict = classifier.predict
            classifier.predict = MethodType(lambda self,x,logits=False: ReverseSigmoid._numpy_reverse_sigmoid_addition(self._predict(x,logits), max_perturb, scale), classifier)
            return classifier
