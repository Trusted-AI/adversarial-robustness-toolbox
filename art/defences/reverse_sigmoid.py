...
class ReverseSigmoid(NetworkManipulator):
    ...
    def __call__(self, classifier, max_perturb=1.0, scale=0.1, input_layer=0, output_layer=0, streamlined=False):
        ...
        if streamlined:
            ...
            if isinstance(classifier, KerasImageClassifier):
                ...
                return KerasImageClassifier(classifier.clip_values, manipulated_model, use_logits=False,
            ...
            if isinstance(classifier, ImageClassifier):
                # Note that the Numpy version makes in-place change to the given classifier.
                ...
                raise ValueError('The reverse sigmoid defence only supports image classifiers.')
