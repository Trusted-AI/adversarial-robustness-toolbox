...
class DataframeWrapper(np.ndarray):
    ...
    __dataframe = None #The underlying dataframe used to create the array. This can change due to changes in the numpy array such as slicing. So long as the # of columns remains consistent and the array is 2D, the dataframe should remain the same.
    ...
    def __array_finalize__(self, obj):
        ...
        if(self.__dataframe is not None):
            ...
            if(len(self_shape) == 2 and self_shape[1] == df_shape[1]):
                ...
                logger.warning("There is an inconsisent mapping between the dataframe and the numpy array likely due to mismatched shapes. Dataframe shape: %s Array shape: %s", df_shape ,self_shape)
