...
def get_and_process_input(return_batch=False):
    """
    Helper function to load relevant data.
    """
    import torch
...
def test_grad_equivalence(max_iter, art_warning):
    """
    Test that the gradient from using ART tools matches that obtained by manual calculation.
    """
    try:
        import torch
...
def test_perturbation_equivalence(to_batch, art_warning):
    """
...
@pytest.mark.only_with_platform("huggingface")
@pytest.mark.parametrize("to_one_hot", [False, True])
def test_attack_functionality(art_warning, to_one_hot):
    """
    Check that the attack results in valid adversarial examples which evade the model.
    """
    try:
        import torch
